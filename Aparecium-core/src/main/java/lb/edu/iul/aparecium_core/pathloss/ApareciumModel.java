...
public class ApareciumModel implements PathLossModel {

    double breakPointDistance;
    double losExponent;
    double nLosExponent;

    public ApareciumModel(double breakPointDistance, double losExponent, double nLosExponent){
    ...
    public double estimatePathLoss(Location receiver, AccessPoint accessPoint, List<Wall> walls) {
        ...
        pathLoss += new Random().nextGaussian() * 2;
        pathLoss += 10 * losExponent * Math.log10(distance) + 10 * (nLosExponent - losExponent) * Math.log10(distance/breakPointDistance);
        for (Wall wall : walls) {
    ...
    public double estimateDistance(double requiredSignalPower, double pathLoss1m, double transmissionPower, List<Wall> walls) {
        double pathLoss = transmissionPower - pathLoss1m - requiredSignalPower;
        pathLoss -= new Random().nextGaussian() * 2;
        for (Wall wall : walls) {
            pathLoss -= wall.getAttenuation();
        }
        return Math.pow(10, (pathLoss/10 + (nLosExponent - losExponent) * Math.log10(breakPointDistance)) / nLosExponent);
    }
}
